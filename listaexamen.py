...
class Lista():
    ...
    def barrido_nombre(self):
        aux=self.__inicio
        while(aux is not None):
            if(("Tyrannosaurus Rex" in aux.info.name or "Spinosaurus" in aux.info.name or "Giganotosaurus" in aux.info.name) and ("critical" in aux.info.alert_level or "high" in aux.info.alert_level )):
                print(aux.info)
            aux = aux.sig
